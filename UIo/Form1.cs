...
public partial class Form1 : Form
{
    ...
    public FileStream configFile = new FileStream("settings.xml", FileMode.OpenOrCreate);
    public SettingsManager? settManager;
    public void LoadSettings()
    {
        try {
            settings = new XmlSerializer(typeof(Settings)).Deserialize(configFile) as Settings ?? settings;
        }
        catch { }

        settManager ??= new SettingsManager(settings, this);
        settManager.Load();
    }
    ...
    private void btn_beginAlg_Click(object sender, EventArgs e)
    {
        if (!algRunning) {
            ...
            new Thread(() => {
				...
				bool runEnd = RunAlgorithm(settings, false);
                if (runEnd)
                    EndAlgorithm();
                ClearAlgorithmData();
                btn_beginAlg.Invoke(() => {
